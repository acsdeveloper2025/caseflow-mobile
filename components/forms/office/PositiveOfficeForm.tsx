...
const PositiveOfficeForm: React.FC<PositiveOfficeFormProps> = ({ caseData }) => {
  ...
  const isFormValid = useMemo(() => {
    ...
    if (report.officeStatus === OfficeStatusOffice.Opened) {
        const openedFields: (keyof PositiveOfficeReportData)[] = [
            'metPerson', 'designation', 'workingPeriod', 'applicantDesignation', 'workingStatus', 'applicantWorkingPremises',
            'officeType', 'companyNatureOfBusiness', 'staffStrength', 'staffSeen', 'officeApproxArea',
            'companyNamePlateStatus', 'documentShown', 'tpcMetPerson1', 'nameOfTpc1', 'tpcConfirmation1',
            'tpcMetPerson2', 'nameOfTpc2', 'tpcConfirmation2', 'establishmentPeriod'
        ];
  ...
  const handleImagesChange = (images: CapturedImage[]) => {
    updatePositiveOfficeReport(caseData.id, { images });
  };
      ...
      {report.officeStatus === OfficeStatusOffice.Opened && (
        <div className="p-4 bg-gray-900/50 rounded-lg space-y-4 border border-dark-border">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <SelectField label="Applicant Designation" id="applicantDesignation" name="applicantDesignation" value={report.applicantDesignation || ''} onChange={handleChange} disabled={isReadOnly}><option value="">Select...</option>{options.designation}</SelectField>
            ...
            <SelectField label="Applicant Working Premises" id="applicantWorkingPremises" name="applicantWorkingPremises" value={report.applicantWorkingPremises || ''} onChange={handleChange} disabled={isReadOnly}><option value="">Select...</option>{options.applicantWorkingPremises}</SelectField>
            {report.applicantWorkingPremises === ApplicantWorkingPremisesOffice.DifferentLocation && <FormField label="Sitting Location" id="sittingLocation" name="sittingLocation" value={report.sittingLocation} onChange={handleChange} disabled={isReadOnly} />}
            <SelectField label="Office Type" id="officeType" name="officeType" value={report.officeType || ''} onChange={handleChange} disabled={isReadOnly}><option value="">Select...</option>{options.officeType}</SelectField>

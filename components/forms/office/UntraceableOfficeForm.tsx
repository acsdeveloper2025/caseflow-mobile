...
const UntraceableOfficeForm: React.FC<UntraceableOfficeFormProps> = ({ caseData }) => {
  ...
  const handleFormDataChange = (formData: any) => {
    if (!isReadOnly) {
      updateUntraceableOfficeReport(caseData.id, formData);
    }
  };

  const handleAutoSaveImagesChange = (images: CapturedImage[]) => {
    if (!isReadOnly && report) {
      updateUntraceableOfficeReport(caseData.id, { ...report, images });
    }
  };

  const handleDataRestored = (data: any) => {
    if (!isReadOnly && data.formData) {
      updateUntraceableOfficeReport(caseData.id, data.formData);
    }
  };
  ...
  const handleImagesChange = (images: CapturedImage[]) => {
    updateUntraceableOfficeReport(caseData.id, { images });
  };

  const handleSelfieImagesChange = (selfieImages: CapturedImage[]) => {
    updateUntraceableOfficeReport(caseData.id, { selfieImages });
  };

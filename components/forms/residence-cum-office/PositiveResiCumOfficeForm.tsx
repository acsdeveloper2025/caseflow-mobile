...
const PositiveResiCumOfficeForm: React.FC<PositiveResiCumOfficeFormProps> = ({ caseData }) => {
  ...
  const isFormValid = useMemo(() => {
    ...
    if (report.resiCumOfficeStatus === ResiCumOfficeStatus.Open) {
        const openedFields: (keyof ResiCumOfficeReportData)[] = [
            'residenceSetup', 'businessSetup', 'metPerson', 'relation', 'stayingPeriod', 'stayingStatus',
            'companyNatureOfBusiness', 'businessPeriod', 'businessStatus', 'businessLocation', 'approxArea',
            'documentShownStatus', 'tpcMetPerson1', 'tpcName1', 'tpcConfirmation1', 'tpcMetPerson2', 'tpcName2', 'tpcConfirmation2'
        ];
    ...
    <div className="space-y-4 pt-4 border-t border-dark-border">
        ...
        {report.resiCumOfficeStatus === ResiCumOfficeStatus.Open && (
            <div className="p-4 bg-gray-900/50 rounded-lg space-y-4 border border-dark-border">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <SelectField label="Residence Setup" id="residenceSetup" name="residenceSetup" value={report.residenceSetup || ''} onChange={handleChange} disabled={isReadOnly}><option value="">Select...</option>{options.sightStatus}</SelectField>
                    <SelectField label="Business Setup" id="businessSetup" name="businessSetup" value={report.businessSetup || ''} onChange={handleChange} disabled={isReadOnly}><option value="">Select...</option>{options.sightStatus}</SelectField>
                    <FormField label="Met Person" id="metPerson" name="metPerson" value={report.metPerson} onChange={handleChange} disabled={isReadOnly} />
                    <SelectField label="Relation" id="relation" name="relation" value={report.relation || ''} onChange={handleChange} disabled={isReadOnly}><option value="">Select...</option>{options.relation}</SelectField>
                    ...
                    <SelectField label="Business Status" id="businessStatus" name="businessStatus" value={report.businessStatus || ''} onChange={handleChange} disabled={isReadOnly}><option value="">Select...</option>{options.businessStatus}</SelectField>
                    <SelectField label="Business Location" id="businessLocation" name="businessLocation" value={report.businessLocation || ''} onChange={handleChange} disabled={isReadOnly}><option value="">Select...</option>{options.businessLocation}</SelectField>
                    {report.businessLocation === BusinessLocation.DifferentAddress && <FormField label="Business Operating Address" id="businessOperatingAddress" name="businessOperatingAddress" value={report.businessOperatingAddress} onChange={handleChange} disabled={isReadOnly} />}
